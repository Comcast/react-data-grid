...
test('should allow dragFill if onFill is specified', async () => {
  ...
  await expect.element(page.getCellsAtRowIndex(0).nth(0)).toHaveFocus();
  await expect.element(page.getCellsAtRowIndex(1).nth(0)).toHaveTextContent('a1');
  await expect.element(page.getCellsAtRowIndex(2).nth(0)).toHaveTextContent('a1');
  await expect.element(page.getCellsAtRowIndex(3).nth(0)).toHaveTextContent('a4'); // readonly cell
});
...
test('should update single row using mouse', async () => {
  ...
  await expect.element(page.getCellsAtRowIndex(1).nth(0)).toHaveTextContent('a1');
  await expect.element(page.getCellsAtRowIndex(2).nth(0)).toHaveTextContent('a3');
  await expect.element(page.getCellsAtRowIndex(0).nth(0)).toHaveFocus();
});
...
test('should allow drag up using mouse', async () => {
  ...
  await expect.element(page.getCellsAtRowIndex(0).nth(0)).toHaveTextContent('a4');
  await expect.element(page.getCellsAtRowIndex(1).nth(0)).toHaveTextContent('a4');
  await expect.element(page.getCellsAtRowIndex(2).nth(0)).toHaveTextContent('a4');
});
